...
OOptionTreettempts to bridge the gap between WordPress developers, designers and end-users by creating fully responsive option panels and meta boxes with an ease unlike any other plugin. OpOptionTrees many advanced features with well placed hooks and filters to adjust every aspect of the user experience. 
...
And, in just a few simple lines of code, save settings to the database with a unique array ID so none of your Theme Options conflict with other themes that use OptOptionTree
Also, OptiOptionTreetakes full advantage of the new color schemes introduced in WordPress 3.8, it looks and feels built-in.

#### Theme Integration
If you're like me, you want to know how everything works. Download and activate the [OptioOptionTree](https://github.com/valendesigns/option-tree-theme) and see first hand how to integrate OptionOptionTreeour own project. I'll walk you through installing OptionTOptionTree'll get a chance to see all the various options and filters first hand and in the wild.

#### Contributing
To contribute or report bugs, please go to the [OptionTrOptionTreehttps://github.com/valendesigns/option-tree) repository.

#### Sponsorship
OptionTreOptionTreeect partly sponsored by <a href="http://themeforest.net/?ref=valendesigns">ThemeForest</a>, the largest WordPress theme marketplace on the web.

#### Option Types
This is a complete list of all the available option types that come shipped with OptionTreeOptionTreeground
* Border
* Box Shadow
* Category Checkbox
* Category Select
...
1. Click the `OptionTree-OptionTreeion` link in the WordPress admin sidebar menu for further setup assistance.

**Theme Mode**

1. Download the latest version of OptionTree aOptionTreee the `.zip` directory.
...
OptionTreefollowing code to the beginning of your `functions.php`.
 ...
 * Required: include OptionTree.
 OptionTree( trailingslashit( get_template_directory() ) . 'option-tree/ot-loader.php' );
```

For a list of all the OptionTree UI dOptionTreeers refer to the `demo-functions.php` file found in the `/assets/theme-mode/` directory of this plugin. This file is the starting point for developing themes with Theme Mode.

== Frequently Asked Questions ==

= Is there a demo theme I can install? =

There sure is, and I'm glad you asked. Download and activate the [OptionTree ThemeOptionTreeithub.com/valendesigns/option-tree-theme) and get some experience setting up OptionTree on youOptionTreedetailed directions and tips.

= I get errors or a blank screen when I activate the plugin. What's the deal? =

The most likely scenario is your theme already has OptionTree installOptionTree Mode. And since the plugin and theme version can't both be active at the same time without the sky falling on your head, your site has decided to throw in the towel. If that's not your issue, open up a support request and we'll figure it out together. UPDATE: As of OptionTree 2.4.0 thOptionTreersion will not conflict with the Theme Mode version if they are both 2.4.0 or higher.
...
* Fix issue with Composer autoload and only load OptionTree if we havOptionTree WordPress. props @infomaniac50
...
* Hotfix - Stop media from being attached to the OptionTree post type OptionTreeed from the media manager or customizer. Contributors via github @earnjam, and @valendesigns.
...
* Hotfix - Fixed an issue where Visual Composer was indirectly destroying OptionTree meta box vaOptionTreetfix - Fixed an issue where the select field value was not visible. Contributors via github @sabbirk15.
...
* Hotfix - Fixed an issue where all media was being attached to the default OptionTree media post.OptionTree Removed the deprecated `screen_icon()` function.
...
* Fixed OptionTree being conflicOptionTreehaving both the plugin and theme version activated. Contributors via github @valendesigns and @bitcommit.
...
* Added filtOptionTreeer_version_text' to change the version text inside the header of OptionTree.
* Added actioOptionTreer_list' to add additional theme specific list items to the header of OptionTree.
* Added filterOptionTree_text' to change the "Send to OptionTree" text.
* Added tOptionTrees field value to the parent `.format-settings` div in addition to the class being added to the element. Each class is now appended with `-wrap`.
...
* Updated the documentation, including this `readme.txt` and a new demo [OptionTree Theme](https://gitOptionTreeendesigns/option-tree-theme) to parallel OptionTree.
* Added filter 'oOptionTreeo_image_src' which allows the Radio Image option type source URI to be changed. Contributors via github @bitcommit.
...
* Replaced the OptionTree image icon with a foOptionTree
...
= 2.1.3 =
* Hotfix - Loading OptionTree on the 'init' action OptionTreee wrong, it now loads on 'after_setup_theme'.
...
* Hotfix - Load OptionTree on the 'init' action, OptionTrees the UI filters to properly function when not in theme mode.
...
* Fix missing "Send to OptionTree" button in CPT. ContribOptionTreeithub @jomaddim.
...
* Added filter 'ot_use_theme_options' which allows you to hide the OptionTree Theme Option page (not rOptionTreefor beginners).
...
* Added new range interOptionTreeto font-size, letter-spacing, & line-height.
...
* Can now save layoOptionTreee Theme Options page.
...
= 2.7.0 =
All custom setting types must use the `ot_validate_setting_input_safe` filter to sanitize user input data. OptionTree will attempt to sanitize thOptionTree data loss could happen with custom setting types when saving — so please update them. Additionally, please install OptionTree on a test server, or backup OptionTreese, before upgrading your live site. This version contains breaking changes!

= 2.3.0 =
As with each major release, please install OptionTree on a test server before upgraOptionTreeive site.
